...
export default function TopNav(){
  ...
  useEffect(()=>{ const s = auth.getSession(); if(s){ setRole(s.role); setUser(s.userId); } },[]);
  const updateRole = (r: Role)=>{ setRole(r); auth.setSession({ role: r, userId: user, scopes: [] }); location.reload(); };
  return (
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <input className="border p-1 rounded w-36 hidden md:inline" defaultValue={user} onBlur={(e)=>{ setUser(e.target.value); auth.setSession({ role, userId: e.target.value, scopes: [] }); }}/>
        </div>
      </div>
    </header>
  );
