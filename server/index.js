// REPLACE your server.js with this known‑good skeleton.
// It boots all core routes + a simple /test page so you SEE pins immediately.
// Then you can re‑merge the advanced pieces you already have.
...
(async ()=>{
  try{
    if (process.env.SMTP_URL){
      transporter = nodemailer.createTransporter(process.env.SMTP_URL);
...
app.post('/api/email', async (req,res)=>{
  try{
    ...
    const mapPath = (p)=>{ try{
      ...
      return p; }catch{ return p; } };
    const atts = (attachments||[]).map(a=> ({ ...(a||{}), path: mapPath(a.path||a) }));
...
app.post('/api/report/photo', async (req,res)=>{
  try{
    ...
    const doc = new PDFDocument({ size:'LETTER', margin:36 });
    const ws = fs.createWriteStream(outPath); doc.pipe(ws);
    ...
    for (let i=0;i<photos.length;i++){
      ...
      try{
        let buf = null;
        if (/^https?:/i.test(url)){
          const r = await fetch(url); buf = Buffer.from(await r.arrayBuffer());
        } else {
          const fp = url.startsWith('/uploads/')? path.join(UPLOAD_DIR, path.basename(url)) : url;
          buf = fs.readFileSync(fp);
        }
        doc.image(buf, { fit:[540,540], align:'center', valign:'center' });
      }catch{}
      if (note) doc.moveDown(0.5).fontSize(11).text(note);
    }
...
app.get('/api/geocode', async (req,res)=>{
  try{
    const address = (req.query.address||'').trim(); if (!address) return res.json({});
...
const writeSLA=(d)=> fs.writeFileSync(SLA_PATH, JSON.stringify(d,null,2));
...
async function notify({ subject, html }){ try{ if (transporter){ await transporter.sendMail({ from: process.env.SMTP_FROM||process.env.SMTP_USER, to: process.env.ALERT_TO||'strategiclandmgmt@gmail.com', subject, html }); } }catch{} }
...
const writeDrone=(d)=> fs.writeFileSync(DRONE_PATH, JSON.stringify(d,null,2));
const readLeads=()=> JSON.parse(fs.readFileSync(LEADS_PATH,'utf8'));
const writeLeads=(d)=> fs.writeFileSync(LEADS_PATH, JSON.stringify(d,null,2));
...
async function reverseGeocode(lat,lng){ try{ const u=`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`; const r=await fetch(u,{ headers:{'User-Agent':'StormOps/1.0 (contact: strategiclandmgmt@gmail.com)'}}); const j=await r.json(); return j?.display_name||null; }catch{ return null; } }

async function normalizeDrone(body){
  const now=Date.now();
  ...
  if (provider.includes('votix')){
    ...
    if ((!lat||!lng) && address){
      // forward geocode
      try{ const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}`; const r = await fetch(url,{ headers:{'User-Agent':'StormOps/1.0 (contact: strategiclandmgmt@gmail.com)'}}); const j=await r.json(); if (j?.[0]){ return { id:`drone:${now}`, provider:'votix', lat:Number(j[0].lat), lng:Number(j[0].lon), address, tags, image, stream, ts: Number(body.timestamp||now) }; } }catch{}
    }
  ...
  if ((!lat||!lng) && address){ try{ const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}`; const r = await fetch(url,{ headers:{'User-Agent':'StormOps/1.0 (contact: strategiclandmgmt@gmail.com)'}}); const j=await r.json(); if (j?.[0]){ lat=Number(j[0].lat); lng=Number(j[0].lon); } }catch{}
  }
  if (lat&&lng && !address) address = await reverseGeocode(lat,lng);
  if (!lat||!lng) return null;
...
function sseBroadcast(evt){ const data = `data: ${JSON.stringify(evt)}\n\n`; for (const c of sseClients){ try{ c.write(data); }catch{} } }
...
const writeCustomers=(d)=> fs.writeFileSync(CUSTOMERS_PATH, JSON.stringify(d,null,2));
app.post('/api/leads/accept', async (req,res)=>{
  try{
    let body=''; req.on('data', ch=> body+=ch); req.on('end', ()=>{
      const { id } = JSON.parse(body||'{}'); if (!id) return res.status(400).json({ error:'id_required' });
      const leads = readLeads(); const idx = (leads.items||[]).findIndex(x=>x.id===id); if (idx<0) return res.status(404).json({ error:'not_found' });
      const lead = leads.items[idx]; leads.items.splice(idx,1); writeLeads(leads);
      const custDb = readCustomers();
      const customer = { id:`c:${Date.now()}`, name:'Unknown Owner', address: lead.address || `${lead.lat}, ${lead.lng}`, lat: lead.lat, lng: lead.lng, status:'new', tags: lead.tags||[], docs: lead.image?[{ name:'lead.jpg', url: lead.image, caption:(lead.tags||[]).join(', ') }]:[], messages:[], timeline:[{ ts:Date.now(), type:'lead_accepted', text:'From drone' }] };
