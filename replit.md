...
- **Maxar/SpyMeSat Satellite Intelligence**: Integrated into Eyes in the Sky module. Features Maxar high-resolution imagery search (WorldView-3 0.31m, GeoEye-1, Legion constellation), SpyMeSat satellite overflight tracking (72-hour window with imaging capability indicators), archive imagery marketplace with per-km² pricing, satellite tasking requests (routine/priority/urgent/critical with estimated delivery), before/after storm damage change detection (roof damage, vegetation loss, debris field, flood extent, structural collapse, road blockage), and full constellation status monitoring. Backend service at `server/services/maxarSatellite.ts`, API routes at `/api/maxar/*`. Supports live Maxar/SpyMeSat API keys with simulation fallback.
